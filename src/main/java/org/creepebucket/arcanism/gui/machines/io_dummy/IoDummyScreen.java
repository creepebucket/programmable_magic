...
public class IoDummyScreen extends MachineScreen<IoDummyMenu> {
	...
	public void buildWidget() {
		...
		contentWindow = new MachineWidgets.InformationWindowWidget(fromCenter(-100, -105),
				fromTopLeft(200, 100), Component.translatable("gui.arcanism.machine.io_dummy.loading"), 200, 100);
	...
	public void buildWindowContent() {
		contentWindow.name = Component.translatable("gui.arcanism.machine.io_dummy." + menu.ioType.get());
		...
		if (menu.ioType.get().startsWith("item")) {
			...
			upper.addChild(new TextWidget(fromCenterTop(4, 15), Component.translatable("gui.arcanism.machine.io_dummy.fluid_access")).applyTheme(LABEL_TEXT));
			...
			bottom.addChild(new TextWidget(fromBottomLeft(7, -20), Component.translatable("gui.arcanism.machine.io_dummy.fluid_storage")).applyTheme(LABEL_TEXT).bottomAlignY());
			...
			bottom.addChild(new TextWidget(fromCenterBottom(4, -20), Component.translatable("gui.arcanism.machine.io_dummy.max_cache")).applyTheme(LABEL_TEXT).bottomAlignY());
