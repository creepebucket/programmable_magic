...
import org.creepebucket.programmable_magic.mananet.machines.generator.heat_exchanger.HeatExchangerBlockEntity;
import org.creepebucket.programmable_magic.registries.ModMenuTypes;

public class HeatExchangerMenu extends MachineMenu {
	public DynamicValue<Double> power;
	public DynamicValue<Double> powerFact;
	public boolean enabled_synced;

	public HeatExchangerMenu(int containerId, Inventory playerInv, RegistryFriendlyByteBuf extra) {
		this(containerId, playerInv, extra.readBlockPos());
	}

	public HeatExchangerMenu(int containerId, Inventory playerInv, BlockPos pos) {
		this(containerId, playerInv);
		setBlockPos(pos);
	}

	public HeatExchangerMenu(int containerId, Inventory playerInv) {
		this(containerId, playerInv, InteractionHand.MAIN_HAND);
	}

	public HeatExchangerMenu(int containerId, Inventory playerInv, InteractionHand hand) {
		super(ModMenuTypes.HEAT_EXCHANGER_MENU.get(), containerId, playerInv, hand, Menu::init);
	}

	protected HeatExchangerMenu(MenuType<?> type, int containerId, Inventory playerInv, InteractionHand hand, Definition definition) {
	...
	protected void onNetworkSynced() {
		var blockEntity = (HeatExchangerBlockEntity) playerInv.player.level().getBlockEntity(pos);
