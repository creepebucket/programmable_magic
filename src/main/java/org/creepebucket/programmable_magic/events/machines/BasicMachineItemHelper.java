...
public class BasicMachineItemHelper {
    ...
    public static @Nullable Held get_held_basic_machine(Player player) {
        ...
        if (stack.getItem() instanceof BlockItem bi && (bi.getBlock() instanceof BasicMachine || bi.getBlock() instanceof BasicManaConnector)) return new Held(InteractionHand.MAIN_HAND, stack, bi);

        stack = player.getOffhandItem();
        if (stack.getItem() instanceof BlockItem bi && (bi.getBlock() instanceof BasicMachine || bi.getBlock() instanceof BasicManaConnector)) return new Held(InteractionHand.OFF_HAND, stack, bi);
